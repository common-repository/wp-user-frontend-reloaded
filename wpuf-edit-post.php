...
class WPUF_Edit_Post {
                ...
                <ul class="wpuf-post-form">
                    ...
                    <?php if ( wpuf_get_option( 'allow_cats' ) == 'on' ) { ?>
                        <li>
							...
							$default_taxonomy = get_option('wpuf_frontend_posting');
							$default_taxonomy = $default_taxonomy['default_taxonomy'];
									
                            $cats = get_the_terms( $curpost->ID, $default_taxonomy );
                            $selected = 0;
                            if ( $cats ) {
                            ...
                            <div class="category-wrap" style="float:left;">
                                <div id="lvl0">
                                    ...
                                    if ( $cat_type == 'normal' ) {
                                        wp_dropdown_categories( 'show_option_none=' . __( '-- Select --', 'wpuf' ) . '&taxonomy='.$default_taxonomy.'&hierarchical=1&hide_empty=0&orderby=name&name=category[]&id=cat&show_count=0&title_li=&use_desc_for_title=1&class=cat requiredField&exclude=' . $exclude . '&selected=' . $selected );
                                    } else if ( $cat_type == 'ajax' ) {
                                        wp_dropdown_categories( 'show_option_none=' . __( '-- Select --', 'wpuf' ) . '&taxonomy='.$default_taxonomy.'&hierarchical=1&hide_empty=0&orderby=name&name=category[]&id=cat-ajax&show_count=0&title_li=&use_desc_for_title=1&class=cat requiredField&depth=1&exclude=' . $exclude . '&selected=' . $selected );
                                    } else {
                                        wpuf_category_checklist( $curpost->ID, false, $default_taxonomy, $exclude);
                                    }
                                    ?>
                                </div>
                            </div>
    ...
    function submit_post() {
        ...
        if ( !$errors ) {
			...
			$default_taxonomy = get_option('wpuf_frontend_posting');
			$default_taxonomy = $default_taxonomy['default_taxonomy'];
			wp_set_post_terms( trim( $_POST['post_id'] ), $post_category, $default_taxonomy );
