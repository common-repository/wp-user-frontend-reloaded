...
function wpuf_taxonomy_fields() {
    ...
    switch ($action) {
        case 'edit':
            wpuf_taxonomy_fields_edit();
            break;

        default:
            wpuf_taxonomy_fields_main();
            break;
    }
}

function wpuf_taxonomy_fields_main() {
    global $taxonomy_fields, $wpdb;
    ?>
    ...
    <div class="wrap wpuf-admin">
        ...
        <form action="" method="post" style="margin-top: 20px;">
            ...
            <table class="widefat meta" style="width: 850px">
                <thead>
                    <tr>
                        <th scope="col" colspan="2" style="font-size: 14px;">Add New Custom Taxonomy</th>
                    </tr>
                </thead>

                <?php wpuf_build_form( $taxonomy_fields ); ?>
        ...
        <table class="widefat meta" style="margin-bottom: 20px;">
            ...
            if ( $wpdb->num_rows > 0 ):
                ...
                $position = array( 'top' => 'Top', 'description' => 'Before Description', 'tag' => 'After Tag', 'bottom' => 'Bottom' );

                $count = 0;
                foreach ($fields as $row):
                    //var_dump( $row );
                    if ( !wpuf_starts_with( $row->field, 'ct_' ) ) {
                        continue;
                    }
                    ?>
                    <tr valign="top" <?php echo ( ($count % 2) == 0) ? 'class="alternate"' : ''; ?>>
                        ...
                        <td>
                            <a href="admin.php?page=wpuf_custom_tax&action=edit&id=<?php echo $row->id; ?>"><img src="<?php echo plugins_url( 'wp-user-frontend-reloaded/images/edit.png' ); ?>"</a>
                            <a href="<?php echo wp_nonce_url( "admin.php?page=wpuf_custom_tax&action=del&id=" . $row->id, 'wpuf_del' ) ?>" onclick="return confirm('Are you sure to delete this field?');"><img src="<?php echo plugins_url( 'wp-user-frontend-reloaded/images/del.png' ); ?>"</a>
                        </td>
...
function wpuf_taxonomy_fields_edit() {
